...
// Copilot's Anthropic Messages endpoint is Anthropic-only by Copilot's own contract, so
...
const collectImageBlocks = (messages: AnthropicMessagesMessage[]): AnthropicMessagesImageBlock[] => {
  const blocks: AnthropicMessagesImageBlock[] = [];
...
const compressInlineImages = async <TPayload extends AnthropicMessagesPayload | AnthropicMessagesCountTokensPayload>(ctx: AnthropicMessagesBoundaryCtx<TPayload>): Promise<void> => {
  ...
  const hasCompressedImage = (block: AnthropicMessagesToolResultContentBlock | AnthropicMessagesUserContentBlock): block is AnthropicMessagesImageBlock =>
    block.type === 'image' && compressedData.has(block);
  const hasCompressedToolResultImage = (
    block: AnthropicMessagesUserContentBlock,
  ): block is AnthropicMessagesToolResultBlock & { content: AnthropicMessagesToolResultContentBlock[] } =>
    block.type === 'tool_result'
    && Array.isArray(block.content)
    && block.content.some(hasCompressedImage);
  const rewriteImage = (block: AnthropicMessagesImageBlock): AnthropicMessagesImageBlock => {
...
// Recompresses every inline base64 image in the outgoing Anthropic Messages payload to
