...
test('modelsField round-trips codexResponsesLite in both directions', () => {
  const [on, off] = modelsField(
    [
      { upstreamModelId: 'gpt-5.6-sol', endpoints: { openaiResponses: {} }, codexResponsesLite: true },
      { upstreamModelId: 'gpt-5.6-luna', endpoints: { openaiChatCompletions: {} }, codexResponsesLite: false },
    ],
    'custom',
  );
...
test('modelsField omits codexResponsesLite when the operator made no call', () => {
  const [model] = modelsField([{ upstreamModelId: 'gpt-prod', endpoints: { openaiChatCompletions: {} } }], 'custom');
...
test('modelsField rejects a non-boolean codexResponsesLite', () => {
  assertThrows(
    () =>
      modelsField(
        [{ upstreamModelId: 'gpt-prod', endpoints: { openaiChatCompletions: {} }, codexResponsesLite: 'true' }],
        'azure',
      ),
    Error,
...
test('modelsField rejects codexResponsesLite on a non-chat model', () => {
  assertThrows(
    () =>
      modelsField(
        [{ upstreamModelId: 'text-embed', endpoints: { openaiEmbeddings: {} }, codexResponsesLite: false }],
        'azure',
      ),
    Error,
