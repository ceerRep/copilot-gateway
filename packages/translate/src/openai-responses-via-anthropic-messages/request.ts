  ...
  for (const item of input.slice(prefixEnd)) {
    ...
    case 'message':
      ...
      case 'developer': {
        ...
        // inline so chronology and position both reach the final Anthropic Messages
  ...
  for (const tool of tools ?? []) {
    ...
    for (const child of tool.tools) {
      ...
      if (typeof child.name !== 'string') {
        throw new TranslatorInputError(`Cannot translate a child without a string name in namespace '${tool.name}' to Anthropic Messages.`);
      }
      ...
      if (child.type === 'custom') {
        ...
        out.push({
          ...
          input_schema: buildCustomToolInputSchema(child.format ?? undefined),
        });
        continue;
      }
...
const translateToolChoice = (
  ...
  if (toolChoice.type === 'function' || toolChoice.type === 'custom') {
    return toolChoice.name ? { type: 'tool', name: qualifiedNamespaceTargetName(namespaceToolNames, toolChoice.name) ?? toolChoice.name } : undefined;
  }
  return undefined;
};
...
export const buildTargetRequest = async (source: OpenAIResponsesRequestPayload, options: BuildTargetRequestOptions = {}): Promise<TargetRequestResult> => {
  ...
  const target: AnthropicMessagesPayload = {
    ...
    stream: true,
    tools,
    tool_choice: translateToolChoice(payload.tool_choice, namespaceToolNames),
    ...(thinking ? { thinking } : {}),
